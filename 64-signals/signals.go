...
to gradefully shutdown when it receives a SIGTERM, or a
command-line tool to stop processing input if it receive a
...
func main() {
	...
	/*
		...
		notifiactions (we'll also make one to notify us when the
		program can exit).
	*/
	...
	/*
		...
		notifications of the the spectified signals.
	*/
	...
	*/
	...
	/*
		...
		(as indicated by the goroutine above sending a value on
		done) and then exit.
	*/
	...
	/*
		...
		signal. By typing ctrl-C (which the terminal shows as ^C)
		we can send a SIGINT signal, causing the program to print
