...
another (perhans non-Go) one. To do this we'll use Go's
...
func main() {
	...
	/*
		For our example we'll exec ls. Go requires an absolute
		path to the binary we want to execute, so we'll use
	...
	*/
	...
	/*
		Exec requires arguments in slice from (as apposed to one
		big string). We'll give ls a few common arguments. Note
	...
	*/
	...
	/*
		...
		be replaced by the /bin/ls -a -l -h process. If there is
		an error we'll get a return value.
	*/
